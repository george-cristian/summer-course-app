package com.summer_course.utils;

/**
 * @author George Cristian
 *
 * Class that holds constants
 *
 */
public class Constants {

    public static final String DASHBOARD_ACTIVITY = "com.summer_course.DashboardActivity";
    public static final String REGISTER_ACTIVITY = "com.summer_course.RegisterActivity";
    public static final String PARTICIPANTS_ACTIVITY = "com.summer_course.ParticipantsActivity";
    public static final String SCHEDULE_ACTIVITY = "com.summer_course.ScheduleActivity";
    public static final String EVENT_PROFILE_ACTIVITY = "com.summer_course.EventProfileActivity";
    public static final String USER_PROFILE_ACTIVITY = "com.summer_course.UserProfileActivity";
    public static final String CHATS_ACTIVITY = "com.summer_course.ChatsActivity";
    public static final String SURVIVAL_GUIDE_ACTIVITY = "com.summer_course.SurvivalGuideActivity";

    public static final String USERS_DATABASE = "users";
    public static final String EVENTS_DATABASE = "events";

    public static final String[] USER_ROLES = {"Participant", "Core-team", "Volunteer", "Admin"};

    public static final String SCHEDULE_EVENT = "schedule_event";
    public static final String USER_PROFILE = "user_profile";

    public static final int PARTICIPANT = 0;
    public static final int VOLUNTEER = 1;
    public static final int CORE_TEAM_MEMBER = 2;
    public static final int ADMIN = 3;

    public static final String FACEBOOK_LINK = "http://facebook.com/profile/";

    public static final String ORGANISERS_CHAT_LINK = "https://chat.whatsapp.com/0jpnpjA9d3WA93rDRKMoZd";
    public static final String PAX_CHAT_LINK = "https://chat.whatsapp.com/Gx9YxqEwiaiBo4bemTKJPt";

    public static final String SURVIVAL_GUIDE_NAME = "Summer_Course_Survival_Guide";

}
